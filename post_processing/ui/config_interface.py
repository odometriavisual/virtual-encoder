...
def show_config_interface(config, folder_path):
    ...
    config.setdefault("Spatial Window", {
    ...
    def load_original_image():
        ...
        try:
            ...
            cv_img_resized = cv2.resize(cv_img_rgb, (600, 450))
    ...
    def update_preview():
        ...
        try:
            ...
            if img_proc_rgb.dtype != np.uint8:
                img_proc_rgb = (img_proc_rgb * 255).astype(np.uint8)
            img_resized = cv2.resize(img_proc_rgb, (600, 450))
    ...
    def apply_and_close():
        ...
        config["Spatial Window"]["method"] = spatial_var.get()
        root.quit()
        root.destroy()
    ...
    tk.Checkbutton(control_frame, text="Aplicar CLAHE", variable=clahe_var,
                   command=on_checkbox_change, font=("Arial", 11)).pack(anchor='w', pady=4)
    tk.Checkbutton(control_frame, text="Aplicar Denoise", variable=denoise_var,
                   command=on_checkbox_change, font=("Arial", 11)).pack(anchor='w', pady=4)

    spatial_var = tk.StringVar(master=root)
    spatial_var.set(config["Spatial Window"]["method"])

    def on_spatial_method_change(*args):
        config["Spatial Window"]["method"] = spatial_var.get()
        update_preview()
        update_param_labels()

    tk.Label(control_frame, text="Janela Espacial (Spatial Window):", font=("Arial", 11)).pack(anchor='w')
    spatial_menu = tk.OptionMenu(control_frame, spatial_var, "raised_cosine", "blackman_harris", None)
    spatial_menu.config(font=("Arial", 11))
    spatial_menu.pack(anchor='w')

    spatial_params_frame = tk.Frame(control_frame)
    spatial_params_frame.pack(anchor='w', padx=20, pady=5)

    def update_param_labels():
        for widget in spatial_params_frame.winfo_children():
            widget.destroy()
        if spatial_var.get() == "blackman_harris":
            for k, v in config["Spatial Window"]["params"].items():
                tk.Label(spatial_params_frame, text=f"{k}: {v}", font=("Arial", 10)).pack(anchor="w")

    spatial_var.trace_add("write", on_spatial_method_change)
    update_param_labels()
