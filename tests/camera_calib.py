...
if __name__ == "__main__":
    ...
    dx_intended, dy_intended = 22.4, 22.4
    ...
    for cx, cy in zip(
        np.arange(start=3, stop=patternSize[0] - 3),
        np.arange(start=3, stop=patternSize[1] - 3),
    ):
        dxs.append(
            centers[cy * patternSize[0] + cx + 1][0]
            - centers[cy * patternSize[0] + cx][0]
        )
        dys.append(
            centers[(cy + 1) * patternSize[0] + cx][0]
            - centers[cy * patternSize[0] + cx][0]
        )

    dx = np.sum(dxs) / len(dxs)
    dy = np.sum(dys) / len(dys)

    print(f"measured: |dx| = {np.linalg.norm(dx):.2f}, |dy| {np.linalg.norm(dy):.2f}")
    ...
    if debug_circles:
        ...
        img2 = img.copy()
        centers2 = np.array(
            [[[x, y]] for x, y in zip(x_expected, y_expected)], dtype=np.float32
        )
