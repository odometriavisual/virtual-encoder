...
class StreamingHandler(server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path == "/":
            ...
            number = float(number)
            picam2.set_controls(
                {"AfMode": controls.AfModeEnum.Manual, "LensPosition": number}
            )  # "LensPosition": number (number -- set the focus position to 1/number, number is any value you set, for example, if you set 2, it means that it will focus on the position of 0.5m.)
            t0 = str(time.time())
            content = texto.encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "text/html")
